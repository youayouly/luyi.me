 ...
 * 访客真人 / 机器判定。
 *
 * 为什么不看 IP：站点挂在 Cloudflare 后面，clientIp() 拿到的是 CF 边缘节点，
 * 同一个人几分钟内就会换 IP（日志里同一台 iPhone 从 172.69.176.135 跳到
 * 172.70.208.100）。所以这里的会话归并和所有信号都只用 UA + 地区 + 路径 + 时间，
 * 即使以后把 IP 修对了，判定结果也不会变差。
 *
 ...
 * vid 认不了：vid = sha256(IP+UA)，而 CF 边缘 IP 会变，同一台设备算出来的 vid 不稳定。
 * 能稳定对上的只有设备四元组。
 */
