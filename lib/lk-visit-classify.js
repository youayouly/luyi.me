...
const { isCloudOrg } = require('./lk-ip-intel.js')
...
/*
 * canvas/WebGL 指纹（docs/.vuepress/utils/deviceFingerprint.js），比 deviceKey
 * 精确得多——不看 UA，看渲染细节，扛得住 iOS 系统升级（deviceKey 里的 os 字段
 * 带具体版本号，升级就变，指纹通常不变）。优先用它判定，deviceKey 只是给没有
 * 指纹的旧数据/极端环境（隐私模式、canvas 被拦）兜底。
 */
function buildOwnerFingerprints(ownerRows) {
  const fps = new Set()
  for (const row of Array.isArray(ownerRows) ? ownerRows : []) {
    if (row.fp) fps.add(row.fp)
  }
  return fps
}
...
function classifyOne(row, session, baseline) {
  ...
  if (row.device === '桌面' && /linux/i.test(String(row.os || '')) && /chrome/i.test(name)) {
  ...
  /*
   * IP 所在网段归属，来自 lib/lk-ip-intel.js 的本地 ASN 表（数据源
   * GeoLite2-ASN，见 scripts/sync-ip-asn.mjs）。写在这里而不是重新查一次：
   * org/asn 是 visit.js 记录时就存进 row 里的，避免同一份数据在读的时候
   * 反复二分查找。表没生成（没配 MAXMIND_LICENSE_KEY）或老数据没有这两个
   * 字段时 row.org 是空字符串，isCloudOrg('') 恒为 false，不影响判定。
   */
  if (isCloudOrg(row.org)) add(-25, `IP 属于云/主机托管商网段（${row.org}）`)
...
function classifyVisits(rows, ownerRows) {
  ...
  const ownerFps = buildOwnerFingerprints(ownerRows)

  return list.map((row, index) => {
    /*
     * 站长优先于其余判定：这一档是「谁」，不是「像不像人」，
     * 混进真人里会让「有没有别人来过」这个唯一想知道的问题失真。
     * 指纹命中优先于 deviceKey——更精确，也更抗系统升级。
     */
    if (row.fp && ownerFps.has(row.fp)) {
      return Object.assign({}, row, {
        verdict: 'owner',
        score: null,
        reasons: ['canvas/WebGL 指纹与「我的设备」登记过的一台一致'],
      })
    }
