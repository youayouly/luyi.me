 ...
 * - 像 GitHub 主页链接或 `@用户名` → 取 `github.com/<user>.png`
 * - 都不像 → 不给头像 URL，前端用昵称首字渲染一个色块
 *
 * GitHub 分支特意只认「明显是 GitHub」的两种写法（完整/半完整链接、`@` 开头的
 * handle），不认裸的用户名——GitHub 用户名规则几乎什么词都能匹配，裸词一旦
 * 当用户名处理，等于把所有原本落进「都不像」分支的输入都改去请求一张多半
 * 不存在的头像图（虽然前端有 `@error` 兜底会退回首字色块，但没必要平白多请求）。
 *
...
/** GitHub 用户名规则：字母数字，中间可插单个连字符，1~39 位，首尾不能是连字符。 */
const GITHUB_NAME = '[a-z\\d](?:[a-z\\d]|-(?=[a-z\\d])){0,38}'
const GITHUB_URL_RE = new RegExp(`^(?:https?://)?(?:www\\.)?github\\.com/(${GITHUB_NAME})/?$`, 'i')
const GITHUB_HANDLE_RE = new RegExp(`^@(${GITHUB_NAME})$`, 'i')
...
/**
 * 把访客填的「邮箱 / QQ 号 / GitHub」解析成身份信息。
 * @param {string} raw
 * @returns {{kind: 'qq'|'email'|'github'|'none', qq: string, email: string, emailHash: string, avatar: string}}
 */
function parseContact(raw) {
  ...
  const gh = value.match(GITHUB_URL_RE) || value.match(GITHUB_HANDLE_RE)
  if (gh) {
    return {
      kind: 'github',
      qq: '',
      email: '',
      emailHash: '',
      avatar: `https://github.com/${gh[1]}.png?size=100`,
    }
  }
